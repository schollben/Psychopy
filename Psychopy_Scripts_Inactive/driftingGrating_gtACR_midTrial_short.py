...
boardNum = 1
baseState = 0
maxPower = 0.3 # maximum power in decimal (ranges from 0 to 1)
#maxPower = 0.15 # maximum power in decimal (ranges from 0 to 1)
gain =  UL.UNI4VOLTS
EngUnits = 1
...
stimOrder = np.empty([numTrials,(numOrientations+1)])
for i in range(0,numTrials):
    stimOrder[i,:] = np.arange(numOrientations+1) # add 1 for blank
    if isRandom:
...
centerPoint = [-54,0]
stimSize = [400,400] #Size of grating in degrees
...
# convert orientations to standard lab notation
        ...
        while clock.getTime()<stimDuration:
            ...
            if clock.getTime()>optoStart and clock.getTime()<optoStart+.1:
