...
numOrientations = 4
orientations = [prefOri, prefOri+180,prefOri, prefOri+180]
...
stimSize = [134,134] #Size of grating in degrees
...
#create grating stim
gratingStim1 = visual.GratingStim(win=myWin,mask='None', tex=textureType,units='deg',
    pos=[-67, 0],size=stimSize, sf=spatialFreq, autoLog=False)

grayStim1 = visual.GratingStim(win=myWin,units='deg',
    pos=[67, 0],size=[134,134], contrast=0, autoLog=False)

gratingStim2 = visual.GratingStim(win=myWin,mask='None', tex=textureType,units='deg',
    pos=[67, 0],size=stimSize, sf=spatialFreq, autoLog=False)

grayStim2 = visual.GratingStim(win=myWin,units='deg',
    pos=[-67, 0],size=[134,134], contrast=0, autoLog=False)
...
for trial in range(0,numTrials): 
    ...
    stimOrder = range(0,len(orientations)+doBlank+doBinoc)
    if nBlank > 1:
        blankID=len(orientations) - doBinoc
    ...
    for stimNumber in stimOrder:
        ...
        if stimNumber == (len(orientations)+doBinoc):
            gratingStim1.setAutoDraw(True)
            grayStim1.setAutoDraw(False)
            gratingStim2.setAutoDraw(True)
            grayStim2.setAutoDraw(False)
            gratingStim1.setContrast(contrast)
            gratingStim2.setContrast(contrast)
            print "\tStim",stimNumber+1," Binoc cond."
        elif stimNumber == len(orientations):
            gratingStim1.setContrast(0)
            gratingStim2.setContrast(0)
            print "\tStim",stimNumber+1," (blank)"
        elif (stimNumber==0) or (stimNumber==1):
            gratingStim1.setAutoDraw(True)
            grayStim1.setAutoDraw(True)
        ...
        elif (stimNumber==2) or (stimNumber==3):
            gratingStim1.setAutoDraw(False)
            grayStim1.setAutoDraw(False)
            gratingStim2.setAutoDraw(True)
            grayStim2.setAutoDraw(True)
            gratingStim2.setContrast(contrast)
            gratingStim2.ori = orientations[stimNumber]-90
            gratingStim1.setContrast(0)
            print "\tStim",stimNumber+1,orientations[stimNumber],'deg'
        clock.reset()
        while clock.getTime()<stimDuration:
            ...
            if stimNumber == len(orientations):
                ...
                gratingStim2.setPhase(startingPhase+clock.getTime()*temporalFreq*1.5)
            else:
                gratingStim1.setPhase(startingPhase+clock.getTime()*temporalFreq)
