...
contVal =         [0.05, 0.20, 0.35, 0.50, 0.65, 0.80, 0.95] # logspace for contrast
SF =                [0.02, 0.04, 0.08, 0.16, 0.32]
...
binocVals = binocVals.reshape(len(contVal)*len(contVal),2) 
...
for trial in range(0,numTrials):
    ...
    for stimNumber in stimIDs:
        ...
        #grating parameters
        gratingStim1.sf = stimulusParams[stimNumber,0]
        gratingStim2.sf = stimulusParams[stimNumber,0]
        phase = stimulusParams[stimNumber,1]
        
        # right eye grating
        meanLuminanceR = meanLumVal
        luminanceRangeR = binocParams[stimNumber,0]
        ...
        # left eye grating
        meanLuminanceL = meanLumVal
        luminanceRangeL = binocParams[stimNumber,1]
