...
orientations1=numpy.concatenate((ori1,ori1,ori1,nulls,ori1));
orientations2=numpy.concatenate((ori1,ori1,ori1,ori1,nulls));
speeds = numpy.concatenate((speed1,speed2,speed3,speed1,speed1))
stiminfo=numpy.array((orientations1,orientations2,speeds))
print stiminfo
...
changeDirectionAt = 0.5  #1 no reversal
stimDuration = 5
isi = 5
isRandom = 1
initialDelay= 1
...
# Grating parameters
#temporalFreq = 4 #set by 'speeds' now
#-----------------#
spatialFreq = 0.1
#-----------------#
contrast = 1
...
stimSize = [134,150] #Size of grating in degrees, basically full field 
# above is changed later though because dont want stimuli creeping into other monitor
centerpos = 67 #visual degrees off by 2 (33.15 x 2)
...
totalStims = len(speeds)+doBlank # total number of stimuli for this set
print "\n",totalStims, "stims will be run for",str(numTrials),"trials."
...
for trial in range(0,numTrials): 
    ...
    for stimNumber in stimOrder:
        ...
        if stimNumber ==blankID:
            ...
            gratingStim1.ori = orientations1[stimNumber]-90
            gratingStim2.ori = orientations2[stimNumber]-90
            temporalFreq = speeds[stimNumber]
            if orientations1[stimNumber]==-1:
                gratingStim1.setContrast(0)
                print "monocular condition"
            if orientations2[stimNumber]==-1:
                gratingStim2.setContrast(0)
                print "monocular condition"
            print "\tStim",stimNumber+1,orientations1[stimNumber],"deg",orientations2[stimNumber],"deg",speeds[stimNumber],"cyc/deg"
        ...
        while clock.getTime()<stimDuration:
            ...
            if clock.getTime()>changeDirectionTimeAt:
                ...
                gratingStim2.setPhase(0 + changeDirectionTimeAt*temporalFreq - (time-changeDirectionTimeAt)*temporalFreq)
            else:
                gratingStim1.setPhase(0 + time*temporalFreq)
                gratingStim2.setPhase(0 + time*temporalFreq)
