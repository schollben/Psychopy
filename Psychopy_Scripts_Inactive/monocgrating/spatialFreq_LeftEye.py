...
# ---------- Stimulus Parameters ---------- #
spatialFreqs = [0.025,0.05,0.1,0.15,0.2,0.25,0.30]
stiminfo = spatialFreqs
...
print "\n",str(len(spatialFreqs)+doBlank), "stims will be run for",str(numTrials),"trials."
if nBlank > 1:
...
for trial in range(0,numTrials): 
    ...
    stimOrder = range(0,len(spatialFreqs)+doBlank)
    if nBlank > 1:
        blankID=len(spatialFreqs)
    ...
    for stimNumber in stimOrder:
        ...
        if stimNumber == len(spatialFreqs):
            gratingStim.setContrast(0)
            print "\tStim",stimNumber+1," (blank)"
        else:
            gratingStim.setContrast(contrast)
            gratingStim.sf = spatialFreqs[stimNumber]
            print "\tStim",stimNumber+1,spatialFreqs[stimNumber],'deg'
        clock.reset()
