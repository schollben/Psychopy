...
stimDuration =1.5 #use 4 because going to use 4 orientations
isi =2
isRandom =1
initialDelay=1
...
print "\n",str(len(temporalFreqs)+doBlank), "stims will be run for",str(numTrials),"trials."
if nBlank > 1:
...
for trial in range(0,numTrials): 
    ...
    stimOrder = range(0,len(temporalFreqs)+doBlank)
    if nBlank > 1:
        blankID=len(temporalFreqs)
    ...
    for stimNumber in stimOrder:
        ...
        if stimNumber == len(temporalFreqs):
            gratingStim.setContrast(0)
            temporalFreq = 1
            print "\tStim",stimNumber+1," (blank)"
        else:
            gratingStim.setContrast(contrast)
            temporalFreq = temporalFreqs[stimNumber]
            print "\tStim",stimNumber+1,temporalFreqs[stimNumber],'cyc/deg'
