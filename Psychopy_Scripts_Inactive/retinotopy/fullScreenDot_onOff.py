...
# RDK parameters
backgroundColor = .1*2-1; #-1.0 is black, 0.0 is gray, and 1.0 is white
...
#make a window
myWin = visual.Window(size=[3840,1080],monitor=mon,fullscr=True,screen=1, allowGUI=False, waitBlanking=False,color=backgroundColor)

directions=numpy.tile(directions,2)
...
rdkStim2 = visual.DotStim(win=myWin, units='pix', nDots=numberOfDots/2, coherence=stimCoherence, fieldPos=centerPoint, fieldSize=stimSize, 
    ...
    colorSpace='rgb', opacity=1.0, contrast=1.0, signalDots='same', noiseDots='direction',autoLog=False);
rdkStim.setAutoDraw(False)
rdkStim2.setAutoDraw(False)
...
for trial in range(0,numTrials): 
    ...
    for stimNumber in stimOrder:
        ...
        if stimNumber == 2*len(directions):
            ...
            if stimNumber < len(directions)/2:
