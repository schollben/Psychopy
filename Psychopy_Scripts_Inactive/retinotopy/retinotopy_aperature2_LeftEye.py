...
gratingStim=visual.GratingStim(myWin,mask='None', units='pix', tex='sqr', 
    size=[240,240], sf=0.008,contrast=1, pos=[0,0],
    autoLog=False)
...
for trial in range(0,numTrials): 
    ...
    for block in allBlocks:
        ...
        for numOri in range(0,len(orientations)):
            ...
            if block<9:
                position = [-1920+(240/2)+240*(block-1) , 540-(240/2)]
            elif block>8 and block<17:
                position = [-1920+(240/2)+240*(block-1-8) ,0]
            else:
                position = [-1920+(240/2)+240*(block-1-16) , -540+(240/2)]
