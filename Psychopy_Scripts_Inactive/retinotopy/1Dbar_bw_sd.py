...
orientation = 157
stimSize = 18
centerPoint = [-68,-0] #screen about 113. deg wide and XX deg tall
...
if barWidthOverlap<1:
    barSize = [barWidth,stimSize*3] #bar length always 1.5x size
else:
...
#set up all bars and combinations
bar1Num1 = numpy.arange(0,numBars-1,1)
bar2Num1 = numpy.arange(0+1,numBars,1)
bar1Num = numpy.concatenate((bar1NumSingle,bar1Num1,bar1NumSingle,bar1Num1))
bar2Num = numpy.concatenate((bar1NumSingle,bar2Num1,bar1NumSingle,bar2Num1))
#set up black and white contrasts
conts = numpy.concatenate(([-1]*(len(bar1Num)/2),[1]*(len(bar1Num)/2)))
#same size? better be...
print len(bar1Num)
print len(conts)
